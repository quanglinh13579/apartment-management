...
const EditContactModal: React.FC<EditContactModalProps> = ({ isOpen, onClose, onSave, contact }) => {
  ...
  const handleSave = () => {
    if (!contact || !name.trim() || !phone.trim()) return;
    onSave({ id: contact.id, name: name.trim(), relationship: relationship.trim(), phone: phone.trim() });
  };
    ...
    <Modal isOpen={isOpen} onClose={handleClose}>
      ...
      <div className="flex flex-col items-center text-center">
        <h2 className="text-xl font-bold text-black mb-2">
          {t('profile.modal_edit_title')}
        </h2>
        <p className="text-sm text-text-muted leading-relaxed mb-6">
          {t('profile.modal_edit_desc')}
        </p>
